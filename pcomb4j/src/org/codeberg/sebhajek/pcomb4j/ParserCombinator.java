...
/**
 * A {@link Parser} decorator that wires together all of the combinator interfaces provided by
 * pcomb4j and adds debug logging around each parse attempt.
 *
 * <p>Instances are created either directly via {@link #ParserCombinator(Logger, Parser)} or, more
 * conveniently, through the fluent {@link Builder} obtained from {@link #withLogger(Logger)}:
 *
 * <pre>{@code
 ...
 */
public class ParserCombinator<Output, Input> implements CombinatorParser<Output, Input> {

  /**
   * Fluent builder used to wrap a {@link Parser} in a {@link ParserCombinator} bound to a
   * particular {@link Logger}.
   */
  public static final class Builder {
    private final Logger logger;

    private Builder(@NonNull final Logger logger) {
      this.logger = logger;
    }

    /**
     * Wraps the given parser in a new {@link ParserCombinator} that logs to the {@link Logger}
  ...
  /**
   * Creates a {@link Builder} that produces {@link ParserCombinator} instances logging to the given
   * {@link Logger}.
   *
   * @param logger the logger to use for debug output during parsing
   * @return a new {@link Builder}
   */
  public static final ParserCombinator.Builder withLogger(@NonNull final Logger logger) {
  ...
  public ParserCombinator(
      @NonNull final Logger logger, @NonNull final Parser<Output, Input> parser) {
    this.logger = logger;
    this.parser = parser;
  }
  ...
  /**
   * Delegates parsing to the wrapped {@link Parser}, logging the current input element at debug
   ...
   */
  @Override
  public ParserResult<Output, Input> parse(@NonNull final ParserInput<Input> parserInput)
      throws ParserError {
