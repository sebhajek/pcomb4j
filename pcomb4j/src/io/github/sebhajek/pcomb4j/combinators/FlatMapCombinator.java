 ...
 *
 * <p>This is the parser-combinator equivalent of {@code flatMap} /
 * {@code bind}: the {@code binder} function is called with the result of
 * this parser and must return the next parser to apply.
 *
...
public interface FlatMapCombinator<Output, Input>
  extends DelegateParser<Output, Input> {
	...
	/**
	 * Creates a parser that first applies this parser, then feeds the result
	 * to {@code binder} to obtain the next parser, and finally applies that
	 * next parser to the remaining input.
	 *
	 * @param binder       a function from this parser's output to the next
	 *                     parser; never {@code null}
