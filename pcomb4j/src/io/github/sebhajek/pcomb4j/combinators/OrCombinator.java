...
/**
 * Combinator that tries this parser first and, on failure, falls back to one
 * or more alternative parsers.
 *
 * <p>Two variants are provided:
 * <ul>
 *   <li>{@link #or(Parser)} / {@link #or(Parser[])} — all alternatives must
 *       produce the <em>same</em> output type; the result is just
 *       {@code Output}.</li>
 *   <li>{@link #orElse(Parser)} — the two branches may produce
 *       <em>different</em> output types; the result is wrapped in a
 *       {@link ParserResult.Either}.</li>
 * </ul>
 *
...
public interface OrCombinator<OutputLeft, Input>
  extends DelegateParser<OutputLeft, Input> {
	...
	/**
	 * Creates a parser that tries this parser, then each of
	 * {@code parserOthers} in turn until one succeeds.
	 *
	 * <p>At least one alternative must be supplied.
	 *
	 * @param parserOthers one or more fallback parsers; none may be
	 *                     {@code null}
	...
	/**
	 * Creates a parser that tries this parser and, on failure, tries
	 * {@code parserOther}.
	 *
	...
	/**
	 * Creates a parser that tries this parser and, on failure, tries
	 * {@code parserOther}. Unlike {@link #or}, the two parsers may produce
	 * <em>different</em> types; the result is a
	 * {@link ParserResult.Either}.
	 *
