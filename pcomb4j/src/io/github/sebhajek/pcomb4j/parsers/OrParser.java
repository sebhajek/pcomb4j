 ...
 *
 * <p>The left parser is tried first; if it fails the right parser is tried
 * on the <em>same</em> (unchanged) input. If both fail a
 * {@link NeitherWasSuccessful} error — containing both child errors — is
 * thrown.
 *
 * <p>Two concrete subclasses are provided:
 * <ul>
 *   <li>{@link Or} — both branches must produce the same output type;
 *       the result is just {@code Output}.</li>
 *   <li>{@link Either} — the branches may produce different types; the
 *       result is a {@link ParserResult.Either}.</li>
 * </ul>
 *
...
public abstract class OrParser<Output, OutputLeft, OutputRight, Input>
	 ...
	 * right parser is tried. If both fail a {@link NeitherWasSuccessful}
	 * error is thrown.
	 *
	...
	public static class Or<Output, Input>
		...
		/**
		 * Tries the left parser; on failure tries the right parser on the
		 * same input.
		 *
	 ...
	 * types.
	 *
	 * <p>The result is wrapped in a {@link ParserResult.Either}: a
	 * {@link ParserResult.Either.Left} if the left branch succeeds or a
	 * {@link ParserResult.Either.Right} if only the right branch succeeds.
	 * If both fail a {@link NeitherWasSuccessful} error is thrown.
	 *
	 * @param <OutputLeft>   the type produced by the left parser
	 * @param <OutputRight>  the type produced by the right parser
	 * @param <Input>        the type of element consumed from the input
	...
	public static class Either<OutputLeft, OutputRight, Input>
		 ...
		 *
		 * @param input the input to parse; never {@code null}
		 * @return a result wrapping the successful branch's value in
		 *         {@link ParserResult.Either.Left} or
		 *         {@link ParserResult.Either.Right}
		 * @throws NeitherWasSuccessful if both branches fail
		 */
		@Override
		...
		private ParserResult<ParserResult.Either<OutputLeft, OutputRight>, Input> getResultRight(
				final ParserInput<Input> input) throws ParserError {
			final var result = getParserRight().parse(input);
			getLogger().trace("right `orElse` parser succeeded: {}", result.result());
		...
		private ParserResult<ParserResult.Either<OutputLeft, OutputRight>, Input> getResultLeft(
				final ParserInput<Input> input) throws ParserError {
			final var result = getParserLeft().parse(input);
			getLogger().trace("left `orElse` parser succeeded: {}", result.result());
			return new ParserResult<>(
