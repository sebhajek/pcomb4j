 ...
 * result against a predicate, failing with {@link NotSatisfied} when the
...
public class FilterParser<Output, Input>
	 ...
	 *
	 * @param parserSource the inner parser whose result is tested;
	 *                     never {@code null}
	 * @param predicate    the condition the parsed value must satisfy;
	 *                     never {@code null}
	...
	public ParserResult<Output, Input> parse(
		...
		if (getPredicate().test(parserResult.result())) {
			logger.debug("`filter` parser succeeded: {}", parserResult.result());
			return parserResult;
		} else {
			logger.debug("`filter` parser failed: {}", parserInput.getCurrent());
